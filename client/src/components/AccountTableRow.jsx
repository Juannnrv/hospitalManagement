...
const AccountTableRow = ({
  ...
  const handleDelete = async () => {
    try {
      ...
      console.log(`Calling onDelete for account ID: ${account.id}`);
      onDelete(account.id);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    const updatedAccount = await updateAccount(account.id, dataToSubmit);
    onUpdate(account.id, updatedAccount.data);
    handleCloseModal();
  };
